...
frontend/
  src/
    ...
    views/        Status · Temp · Extrude · Move · Tune · BedMesh · Files · Macros · Console · Power · Updates · Settings (side-rail destinations)
    locales/      19 namespaced catalog folders (console/control/extrude/files/macros/mesh/move/osk/power/prompt/settings/shell/status/temp/tune/updates)
...
- **Touch-first, one canvas.** The UI is a single proportional canvas: the root font size derives from the viewport (16px at the 1024×600 reference, clamped for legibility), every dimension is authored in `rem`, and each screen lays out inside the visible height (lists scroll internally, controls never do) - so every panel size from 3-inch up shows the identical composition. Big tap targets (min 3.25rem ≈ 52px at reference), the shell owns global affordances (brand, trust ribbon, E-STOP, nav), tools are fullscreen overlays.
- **Device-local display prefs.** Theme and language roam across surfaces via the shared settings store; a panel's own readability does not. UI size (a text/spacing/touch scale via `--ui-*` token multipliers), high-contrast, screen sleep, and backlight brightness live in the webview's `localStorage` (`core/localPrefs.ts`) and apply through `core/theme.ts`. Brightness writes the panel backlight through a tiny native Tauri command (`set_backlight`); screen sleep powers the panel fully off (past the brightness floor) through a companion `set_backlight_power` command, so an idle panel truly goes dark rather than only overlaying black. Both are enabled by a udev rule the kiosk install ships.
