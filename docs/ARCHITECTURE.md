...
- **Device-local display prefs.** Theme and language roam across surfaces via the shared settings store; a panel's own readability does not. UI size (a text/spacing/touch scale via `--ui-*` token multipliers), high-contrast, screen sleep, and backlight brightness live in the webview's `localStorage` (`core/localPrefs.ts`) and apply through `core/theme.ts`. Brightness writes the panel backlight through a tiny native Tauri command (`set_backlight`), enabled by a udev rule the kiosk install ships.
