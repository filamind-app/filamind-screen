...
- **Native app:** the Tauri shell (`src-tauri/`) targets WebKitGTK 4.1 (`webkit2gtk-4.1`) on Linux, with the standard GTK / libsoup3 / Rust prerequisites. The window is configured fullscreen, undecorated, and non-resizable at 1024×600 with a tightened CSP. CI compiles the shell on every push as a guard, and the Release workflow builds the arm64 `.deb` on an arm64 runner and attaches it to the Release under the stable name `filamind-screen-arm64.deb`.
...
- **Touch-first.** Big tap targets (min 44 px), the shell owns global affordances (brand, trust ribbon, E-STOP, nav), tools are fullscreen overlays.
